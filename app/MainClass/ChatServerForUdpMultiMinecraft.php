...
/**
 * サーバー間通信プロトコル（UDP）実装のチャットサーバークラス
 * 
 * オリジナルプロトコル実装のサンプル
 */
class ChatServerForUdpMultiMinecraft extends Console
{
    //--------------------------------------------------------------------------
    // 定数
    //--------------------------------------------------------------------------
     ...
     */
    protected string $identifer = 'app:udpmulti-minecraft-server {port?}{parent_port?}';

    /**
     * @var string コマンド説明
     */
    protected string $description = 'UDPマルチマインクラフト版チャットサーバー';
    ...
    public function exec()
    {
        ...
        // 親ポート番号の設定
        $this->parent_port = config('const.port', $this->parent_port);
